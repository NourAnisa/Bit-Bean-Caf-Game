...
export function createCafe(container,stations){
 ...
 const grainCanvas=document.createElement('canvas');grainCanvas.width=256;grainCanvas.height=256;const gc=grainCanvas.getContext('2d');gc.fillStyle='#b38455';gc.fillRect(0,0,256,256);for(let i=0;i<180;i++){gc.strokeStyle=i%2?'#825d3933':'#f0c68c44';gc.beginPath();gc.moveTo(0,i*1.5);gc.bezierCurveTo(80,i*1.5+Math.sin(i)*5,170,i*1.5-4,256,i*1.5);gc.stroke();}const grain=new T.CanvasTexture(grainCanvas);grain.colorSpace=T.SRGBColorSpace;wood.map=grain;wood.color.set('#ffffff');
 ...
 const customer=new T.Group();scene.add(customer);customer.position.set(3.2,0,-.8);
 ...
 // Dedicated workstation props, floating labels, and first-person hands.
 box(-4.7,1.4,.65,1.1,.8,1.2,dark);box(-4.7,1.85,.65,1.16,.1,1.25,steel);box(-4.7,1.91,.65,.85,.025,.8,black);cyl(-4.7,2.1,.35,.03,.45,steel);box(-4.7,2.32,.55,.06,.06,.4,steel);
 cyl(-.3,2.15,1,.18,.4,steel);cyl(2.2,2.25,.5,.2,.7,steel);box(2.2,2.1,.85,.08,.08,.4,steel);for(let i=0;i<4;i++)cup(1.1,2.1+i*.1,.55);
 const labelSprites=[];stations.forEach(st=>{const c=document.createElement('canvas');c.width=512;c.height=100;const g=c.getContext('2d');g.fillStyle='#142f29';g.fillRect(0,0,512,100);g.strokeStyle='#d8b578';g.strokeRect(2,2,508,96);g.fillStyle='#f1ddba';g.textAlign='center';g.font='28px sans-serif';g.fillText(st.name,256,60);const tex=new T.CanvasTexture(c);tex.colorSpace=T.SRGBColorSpace;const sprite=new T.Sprite(new T.SpriteMaterial({map:tex,depthTest:false}));sprite.position.set(st.x,3.05,1);sprite.scale.set(1.3,.25,1);scene.add(sprite);labelSprites.push(sprite);});
 camera.fov=64;camera.updateProjectionMatrix();scene.add(camera);const hand=new T.Group();camera.add(hand);hand.position.set(.32,-.36,-.65);cyl(0,-.12,.07,.065,.3,skin,hand);sphere(0,.005,0,.07,skin,hand);const held=new T.Group();hand.add(held);cup(0,.12,-.025,held);const liquid=cyl(0,.275,-.025,.13,.012,mat('#4e2817'),held);hand.visible=false;
 const pour=mesh(new T.CylinderGeometry(.009,.013,.5,12),mat('#eee3c9'),-2.1,2.1,1.05);pour.visible=false;
 let fps=false,active=false,state=null,yaw=0,pitch=-.22,px=1.1,pz=2.7,previous=0,walk=0,depart=0;
 const keys=new Set();let drag=null;container.addEventListener('pointerdown',e=>{if(!fps)return;drag={x:e.clientX,y:e.clientY};renderer.domElement.setPointerCapture(e.pointerId);});container.addEventListener('pointermove',e=>{if(!drag||!active)return;yaw-=(e.clientX-drag.x)*.004;pitch=T.MathUtils.clamp(pitch-(e.clientY-drag.y)*.003,-.75,.45);drag={x:e.clientX,y:e.clientY};});container.addEventListener('pointerup',()=>drag=null);container.addEventListener('pointercancel',()=>drag=null);
 window.addEventListener('keydown',e=>{if(active&&['KeyW','KeyA','KeyS','KeyD'].includes(e.code)){keys.add(e.code);e.preventDefault();}});window.addEventListener('keyup',e=>keys.delete(e.code));window.addEventListener('blur',()=>keys.clear());
 function nearest(){if(!fps)return null;return stations.filter(st=>Math.abs(st.x-px)<.68&&pz<3.3).sort((a,b)=>Math.abs(a.x-px)-Math.abs(b.x-px))[0]||null;}
 function frame(t){const time=t*.001,dt=Math.min((t-previous)/1000||0,.05);previous=t;if(fps){if(active){let dx=(keys.has('KeyD')?1:0)-(keys.has('KeyA')?1:0),dz=(keys.has('KeyS')?1:0)-(keys.has('KeyW')?1:0);const norm=Math.hypot(dx,dz)||1;dx/=norm;dz/=norm;px=T.MathUtils.clamp(px+(dx*Math.cos(yaw)+dz*Math.sin(yaw))*dt*2.6,-5.25,3.8);pz=T.MathUtils.clamp(pz+(-dx*Math.sin(yaw)+dz*Math.cos(yaw))*dt*2.6,2,3.9);if(dx||dz)walk+=dt*9;}camera.position.set(px,2.65+Math.sin(walk)*.018,pz);camera.rotation.set(pitch,yaw,0,'YXZ');hand.visible=!!state?.cup;hand.rotation.z=state?.job?.type==='milk'?-.25:Math.sin(walk)*.02;liquid.material.color.set(state?.cup?.addition==='milk'?'#c69a68':'#4e2817');liquid.visible=!!state?.cup?.espresso;labelSprites.forEach(o=>o.visible=true);}else{camera.position.set(10,9,13);camera.lookAt(target);hand.visible=false;labelSprites.forEach(o=>o.visible=false);}customer.position.y=Math.sin(time*1.6)*.016;customer.position.x=3.2+depart;if(active)depart=Math.max(0,depart-dt*2);customer.rotation.y=Math.sin(time*.5)*.1;const brewing=state?.job?.type==='brew';pour.visible=!!state?.job&&['brew','milk','water'].includes(state.job.type);if(pour.visible){pour.position.set(brewing?-2.1:state.job.type==='milk'?-.3:2.2,2.1,1.05);pour.material.color.set(brewing?'#583015':state.job.type==='milk'?'#f1e7ca':'#a9c9ce');}steam.forEach((o,i)=>{const q=(time*.3+i*.2)%1;o.position.set(-2.1+Math.sin(time+i)*.04,2.35+q*.7,1.05);o.material.opacity=brewing?(1-q)*.3:.025;o.scale.setScalar(.6+q*1.6);});renderer.render(scene,camera);}
 ...
 return{start(){fps=true;px=1.1;pz=2.7;yaw=0;pitch=-.22;},menu(){fps=false;active=false;keys.clear();},sync(s,running){state=s;active=running;if(!running)keys.clear();},nearest,key(k,on){on?keys.add(k):keys.delete(k);},look(d){yaw+=d;},quality(high){renderer.setPixelRatio(Math.min(devicePixelRatio,high?2:1));renderer.shadowMap.enabled=high;scene.traverse(o=>{if(o.material)o.material.needsUpdate=true;});},serve(){depart=2.5;}};
}
